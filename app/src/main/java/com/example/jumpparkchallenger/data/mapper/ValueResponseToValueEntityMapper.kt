...
class ValueResponseToValueEntityMapper(private val priceEstablishmentId: Int) : Mapper<ValueResponseData, ValueDetailEntity> {
    override fun map(input: ValueResponseData): ValueDetailEntity {
        return ValueDetailEntity(
            price = input.price,
            period = input.period,
            priceType = input.type,
            tablePriceId = priceEstablishmentId
        )
    }
}